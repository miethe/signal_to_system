 ...
 * check-prose.mjs
 *
 * Editorial gate for the owner's authored prose: fails the build when
 * content under src/content/posts/ or src/content/series/ contains an
 * em dash (U+2014), the most-named AI "tell" for this publication. Also
 * emits an advisory (non-failing) count of en dashes (U+2013) used
 * mid-sentence, and advisory counts of other named patterns that are not
 * mechanically checkable without false positives.
 *
 * SCOPE — posts/ and series/ ONLY. Deliberately excluded:
 *   - src/content/stories/  — machine-generated by the `op story` pipeline.
 *     At the time this scope was set, stories/ carried 102 em dashes; a
 *     content sweep there would not durably fix anything because the
 *     pipeline regenerates the content. Fix the generator, not the output,
 *     if stories/ prose needs this treatment.
 *   - src/content/ui/       — component documentation, not published prose.
 *     It is not subject to the site's voice/em-dash editorial standard.
 *
 * Do not widen SCAN_ROOTS below to cover stories/ or ui/ without revisiting
 ...
 *
 * Usage:
 *   node scripts/check-prose.mjs            # scan posts/ and series/
 *   npm run check:prose
 */
...
// Gate scope: owner-authored prose only. See header comment above for why
// stories/ and ui/ are intentionally excluded.
const SCAN_ROOTS = [join(CONTENT_DIR, "posts"), join(CONTENT_DIR, "series")];
...
function main() {
  ...
  if (files.length === 0) {
    const rootNames = SCAN_ROOTS.map((r) => relative(ROOT, r)).join(", ");
    console.warn(`check-prose: no content files found under ${rootNames}`);
    process.exit(0);
  }
  ...
  if (allFailures.length > 0) {
    console.error(
      `\ncheck-prose: FAILED — ${allFailures.length} em dash (—, U+2014) occurrence(s) found in src/content/posts/ or src/content/series/.\n`
    );
    console.error(
  ...
  console.log(
    `check-prose: OK — scanned ${files.length} file(s) under src/content/posts/ and src/content/series/, 0 em dashes found.`
  );
}
