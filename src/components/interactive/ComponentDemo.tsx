  ...
  return (
    ...
    >
      ...
      <div className="flex items-center justify-between border-b border-zinc-200 bg-zinc-50 px-4 py-2 dark:border-zinc-700 dark:bg-zinc-900">
        ...
        {/* Tab switcher */}
        <div role="tablist" aria-label="Component Demo Tabs" className="flex rounded-lg border border-zinc-200 dark:border-zinc-700 overflow-hidden text-xs">
          {(["preview", "info"] as const).map((t) => (
            <button
              ...
              role="tab"
              aria-selected={tab === t}
              aria-controls={`panel-${t}-${componentName}`}
              id={`tab-${t}-${componentName}`}
              onClick={() => setTab(t)}
              className={[
      ...
      {tab === "preview" && (
        <div
          role="tabpanel"
          id={`panel-preview-${componentName}`}
          aria-labelledby={`tab-preview-${componentName}`}
      ...
      {tab === "info" && (
        <div
          role="tabpanel"
          id={`panel-info-${componentName}`}
          aria-labelledby={`tab-info-${componentName}`}
