...
export function capsuleMarkdown(capsule) {
  ...
  if (capsule.claims) { out.push('', '## Claims'); for (const item of capsule.claims) { out.push(`- ${escape(item.label)} — ${escape(item.status)}: ${escape(item.statement)}`); for (const limitation of item.limitations ?? []) out.push(`  - Limitation: ${escape(limitation)}`); } }
