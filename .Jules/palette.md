...
## 2025-08-23 - Add standard ARIA tab attributes to interactive components
**Learning:** React tab components often lack standard semantic ARIA roles like `role="tablist"` and `role="tabpanel"` out of the box, defaulting to generic `<div>` layouts. Screen reader users rely on `aria-controls` on the tabs pointing to the IDs of panels and `aria-labelledby` on the panels pointing to the tab IDs to understand the structure.
**Action:** When inspecting or modifying un-styled tabs, always check for the full standard ARIA structural pattern (`tablist`, `tab`, `tabpanel`, `aria-selected`, `aria-controls`, `aria-labelledby`).
