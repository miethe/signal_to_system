...
## 2024-03-24 - Use React useId() for ARIA attributes in reusable components
**Learning:** When adding ARIA ID linkages (like `aria-controls` and `aria-labelledby`) to reusable components (e.g., `ComponentDemo`), hardcoding IDs creates accessibility issues if the component is rendered multiple times on the same page.
**Action:** Always generate unique component instance prefixes using the React `useId()` hook and apply them to ARIA ID attributes.
